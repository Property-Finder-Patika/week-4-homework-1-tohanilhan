...
import (
	...
	mathFunction "week-4-homework-1-tohanilhan/calculator/math"
)
...
func (c Calculator) doCalculation(name string, arg float64) (float64, error) {
	...
	for _, f := range c.functions {
		...
		if strings.ToUpper(f.GetName()) == strings.ToUpper(name) {
			result = f.Calculate(arg)
			return result, nil
		}
	}
	return result, fmt.Errorf("Function %s not found", name)
}
...
func startCalculator() {
	...
	for flag {
		var fName string
		var arg float64
		fmt.Println("> Enter name of the calculation or press x to exit the program:")
	...
	fmt.Println("Calculator stopped. Goodbye!")
}
